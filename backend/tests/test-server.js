...
// Add this function to parse WebDriverIO output
function parseWebDriverIOOutput(output) {
    const results = {
        totalTests: 0,
        passedTests: 0,
        failedTests: 0,
    ...
    for (const line of lines) {
        // Parse summary lines like "1 passing (2s)"
        const passingMatch = line.match(/(\d+)\s+passing/);
        if (passingMatch) {
            results.passedTests = parseInt(passingMatch[1]);
        }
        
        const failingMatch = line.match(/(\d+)\s+failing/);
        if (failingMatch) {
            results.failedTests = parseInt(failingMatch[1]);
        }
    }
...
app.post('/run-test', async (req, res) => {
  ...
  try {
    ...
    testProcess.on('close', (code) => {
      ...
      const parsedResults = parseWebDriverIOOutput(output);
      ...
      const results = {
        ...
        applicationId,
        apkFileName,
        parsedResults // Add parsed test results
      };
