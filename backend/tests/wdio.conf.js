...
exports.config = {
    ...
    //
    specs: [
        // Load tests for specific application if APPLICATION_ID is provided
        process.env.APPLICATION_ID
            ? `./specs/${process.env.APPLICATION_ID}/**/*.js`
            : './specs/**/*.js',
    ],
    // Patterns to exclude.
    exclude: [
    ...
    //
    // Dans la section capabilities
    capabilities: [
        {
            platformName: 'Android',
            'appium:platformVersion': '11',
            'appium:deviceName': 'Android Emulator',
            'appium:app': process.env.APK_FILE_NAME
                ? `/opt/resources/${process.env.APK_FILE_NAME}`
                : '/opt/resources/1746808942489-VodQA.apk',
            ...
            'appium:skipLogcatCapture': false, // Garder les logs pour le debug
            'appium:ignoreUnimportantViews': true,
            'appium:enablePerformanceLogging': false,

            // Paramètres de stabilité supplémentaires
    ...
    waitforTimeout: 30000,
    connectionRetryTimeout: 180000,
    connectionRetryCount: 5, // Plus de tentatives
    //
    ...
    onPrepare: function (config, capabilities) {
        ...
        if (!process.env.APK_FILE_NAME) {
            console.warn(
                'Warning: APK_FILE_NAME environment variable is not set. Appium might not find the APK.'
            );
    ...
    afterTest: async function (test, context, { error, result, duration, passed, retries }) {
        console.log(
            ...
            } in ${duration}ms (retries: ${retries})`
        );
    },
